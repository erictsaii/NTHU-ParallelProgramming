...
__global__ void sobel(unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels, int totalThreads) {
    int x, y, i, v, u;
    int R, G, B;
...
int main(int argc, char** argv) {
    ...
    cudaMalloc((void**)&dev_src, height * width * channels * sizeof(unsigned char));
    cudaMalloc((void**)&dev_dst, height * width * channels * sizeof(unsigned char));

    /* Hint 2 */
    // cudaMemcpy(...) copy source image to device (filter matrix if necessary)
    cudaMemcpy(dev_src, host_s, height * width * channels * sizeof(unsigned char), cudaMemcpyHostToDevice);
    ...
    sobel<<<numBlocks, blockSize>>>(dev_src, dev_dst, height, width, channels, totalThreads);
    cudaDeviceSynchronize();

    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_t, dev_dst, height * width * channels * sizeof(unsigned char), cudaMemcpyDeviceToHost);
